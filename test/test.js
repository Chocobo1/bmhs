...
describe("Unit tests", function() {
    describe("bmhs.generateSkipTable", function() {
        it("Boundary tests", function() {
            ...
            checkFinite("");
            checkFinite("a");
            checkFinite("looooooong string");

            checkFinite(new Uint32Array(0));
            checkFinite(new Uint32Array(1));
            checkFinite(new Uint32Array(255));
        });
    });
    ...
    describe("bmhs.match", function() {
        ...
        it("Boundary tests", function() {
            dataSet.forEach((val) => {
                ...
                const idx = Bmh.BoyerMooreHorspool(pattern).match(corpus);
        ...
        it("Reuse tests", function() {
            dataSet.forEach((val) => {
                ...
                Assert.equal(bmh.match(corpus), bmh.match(corpus));
            });
        });
...
describe("Property tests", function () {
    it("Single integer element", function() {
        Jsc.assertForall(Jsc.nearray(Jsc.integer), (corpus) => {
            ...
            const idx = Bmh.BoyerMooreHorspool([value]).match(corpus, corpusOffset);
